...
function SoftwareEngineeringSheet({ auth, setAuth }) {
  ...
  const fetchStats = async () => {
    try {
      const response = await axios.get(`${API_URL}/progress/summary?topic=${topic}`, getAuthHeaders());
  ...
  const toggleSection = (subTopic) => {
    setExpandedSections(prev => ({ ...prev, [subTopic]: !prev[subTopic] }));
  };
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center">
        <div className="text-[#00ff00] text-xl">Loading Software Engineering concepts...</div>
      </div>
    );
  }
          ...
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-4">
              ...
              <h1 className="text-2xl font-bold">
                <span className="text-[#00ff00]">Software Engineering</span> Concepts
              </h1>
            </div>
            <div className="flex items-center space-x-4">
              ...
              {expandedSections[subTopic] && (
                <div className="space-y-3">
                  {filteredConcepts.map((concept, index) => (
                    <div
                      key={concept._id}
                      className={`bg-[#1a1a1a] border rounded-lg p-4 hover:bg-[#252525] transition-all ${concept.completed ? 'border-[#00ff00]/50' : 'border-[#2a2a2a]'}`}
                    >
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex space-x-4 mt-2">
                            {concept.youtubeLink && concept.youtubeLink !== '' && (
                              <a href={concept.youtubeLink} target="_blank" rel="noopener noreferrer" className="text-sm text-red-400 hover:text-red-300 transition-colors flex items-center space-x-1">
                            ...
                            {concept.notesLink && concept.notesLink !== '' && (
                              <a href={concept.notesLink} target="_blank" rel="noopener noreferrer" className="text-sm text-blue-400 hover:text-blue-300 transition-colors flex items-center space-x-1">
                                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
...
export default SoftwareEngineeringSheet;
